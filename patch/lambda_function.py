...
def parse_transcribe_ouput(Transcribe_jsondata):
    ...
    if "speaker_labels" in rawjsondata["results"].keys():
        ...
        for segment in rawjsondata["results"]["speaker_labels"]["segments"]:
            ...
            if len(segment["items"]) > 0:
                data_for_athena["time"].append(
                    time_conversion(segment["start_time"]))
                ...
                for word in segment["items"]:
                    ...
                    word_result = list(
                        filter(
                            lambda x: x["start_time"] == word["start_time"] and
                            x["end_time"] == word["end_time"],
                            pronunciations,
                        ))

                    result = sorted(word_result[-1]["alternatives"],
                    ...
                    try:
                        word_result_index = rawjsondata["results"][
                            "items"].index(word_result[0])
                        next_item = rawjsondata["results"]["items"][
                            word_result_index + 1]
                        if next_item["type"] == "punctuation":
                            data_for_athena["comment"][-1] += next_item[
    ...
    # Invalid File exiting!
        ...
        print(
            "Need to have speaker identification, Please check the file USE WAV format Audio file for better results"
        )
        return
...
def lambda_handler(event, context):
    ...
    while True:
        status = transcribe.get_transcription_job(
            TranscriptionJobName=audio_transcribed_job)
    ...
    if status['TranscriptionJob']['TranscriptionJobStatus'] in ['FAILED']:
        print(
            'Job Failed issue with Audio file please check audio file and upload proper WAV audio file again'
        )
        response = transcribe.delete_transcription_job(
            TranscriptionJobName=audio_transcribed_job)

        sqs.delete_message(QueueUrl=AudioQ, ReceiptHandle=receipt_handle)
    ...
    header_rec = [
        'TIME', 'speaker_tag', 'SENTENCE', "Sentiment", "Positive", "Negative",
        "Neutral", "Mixed"
    ]
    ...
    response = athena_client.get_named_query(NamedQueryId=AthenaNamedQuery)
    ...
    for i in range(1, 1 + RETRY_COUNT):
        ...
        if query_execution_status == 'SUCCEEDED':
            print("Query Execution Id: " + response['QueryExecutionId'] +
                  " ==> Attempt: " + str(i) + " ==> Query Execution Status: " +
                  query_execution_status)
            break
        elif query_execution_status == 'FAILED':
            raise Exception("Query Execution Id: " +
                            response['QueryExecutionId'] + " ==> Attempt: " +
                            str(i) + " ==> Query Execution Status: " +
                            query_execution_status)
        else:
            print("Query Execution Id: " + response['QueryExecutionId'] +
                  " ==> Attempt: " + str(i) + " ==> Query Execution Status: " +
    ...
    for item, elem in enumerate(sentence):
        ...
        sentiment = comprehend.detect_sentiment(Text=elem, LanguageCode='en')
        rec = [
            timedur[item],
            speaker_tag[item],
            elem,
        ...
        insert_stmt = 'insert into default.transcribe_data values(' + '\'' + audio_transcribed_job + '\'' + ',' + '\'' + timedur[
            item] + '\'' + ',' + '\'' + speaker_tag[
                item] + '\'' + ',' + '\'' + elem.replace("'",
                                                         "''") + '\'' + ')'
    ...
    s3.put_object(Bucket=os.environ['PROCSDBUCKET'],
                  Key=audio_transcribed_job + ".csv",
                  Body=csv_data.getvalue())

    csv_presigned_url = s3.generate_presigned_url('get_object',
                                                  Params={
                                                      'Bucket':
                                                      BUCKET,
                                                      'Key':
                                                      audio_transcribed_job +
                                                      ".csv"
                                                  },
                                                  ExpiresIn=86400)
    json_presigned_url = s3.generate_presigned_url('get_object',
                                                   Params={
                                                       'Bucket':
                                                       BUCKET,
                                                       'Key':
                                                       audio_transcribed_json
                                                   },
                                                   ExpiresIn=86400)

    print('Publish to SNS...')

    JobId = audio_transcribed_job.split("-_-seperater-_-")[0].replace(
        "TranscribeBlogJob-", "")
    message = {
        "JobId": JobId,
    ...
    print(message)
    response = sns.publish(TopicArn=os.environ['TranscribeCompletionSNSTopic'],
